...
from ..patch import xmlpatchsampler as dptpatchsampler
...
class XmlBatchSampler(dptbasebatchsampler.BaseBatchSampler):
    ...
    def __init__(self,
        ...
        Args:
            ...
            label_mode (str): Label generation mode. Accepted values:
                'ASAP': only xml format supported thus far
        ...
        # Initialize members.
        #
        self._name_tag = ''                # Name tag of the batch sampler.
        self._log_name_tag = ''            # Logging name tag of the batch sampler for easy identification in logs.
        self._data_source = {}             # List of PatchSource objects per source category.
        self._category_dist = {}           # Image category distribution ratio in a batch.
        self._label_dist = {}              # Label distribution ratio in a batch.
        self._strict_selection = False     # Strict source item selection.
        self._create_stats = False         # Create missing .stat files.
        self._patch_shapes = {}            # Shape of extracted patches per pixel spacing.
        self._mask_spacing = 0.0           # Mask pixel spacing to use.             mask_spacing (float): Pixel spacing of the masks to process (micrometer).
        self._spacing_tolerance = 0.0      # Tolerance for finding a level for the given pixel spacing.
        self._input_channels = []          # List of channels used in a patch
        self._label_mode = ''              # Label generation mode.
        self._process_count = 0            # Number of worker processes to spawn.
        self._sampler_count = sys.maxsize  # Number of images to use for extraction in a round.
        self._patch_samplers = {}          # Patch sampler collection (either PatchSampler objects or host process identifiers).
        self._data_adapter = None          # Data adapter.
        self._free_label_range = False     # Free label mapping flag: no label range checking if True.
        self._mp_context = None            # Multiprocessing context.
        self._command_pipes = {}           # Pipes for the patch sampler processes.
        self._worker_processes = {}        # Patch sampler worker processes.
        self._response_queue = None        # Response queue.
        self._join_timeout = 0             # Timeout seconds to wait for child processes to join.
        self._response_timeout = 0         # Timeout seconds to wait for inter-process communication responses.
        self._poll_timeout = 0             # Timeout seconds to wait for inter-process communication requests.

        self._setname(name_tag=name_tag)
        self._initlogging()

        # Process the configured parameters.
        #
        self._setdataadapter(data_adapter=data_adapter)
        self._setlabels(label_dist=label_dist, label_mode=label_mode, strict_selection=strict_selection, free_label_range=free_label_range)
        self._setpatchshape(patch_shapes=patch_shapes, mask_spacing=mask_spacing, spacing_tolerance=spacing_tolerance, input_channels=input_channels)
        self._addsource(patch_sources=patch_sources, create_stats=create_stats, category_dist=category_dist)
        self._configuremultiprocessing(process_count=process_count, sampler_count=sampler_count, join_timeout=join_timeout, response_timeout=response_timeout, poll_timeout=poll_timeout)
    ...
    def _setname(self, name_tag):
        ...
        self._log_name_tag = '[{name}] '.format(name=name_tag) if name_tag else ''
    ...
    def _initlogging(self):
        ...
        #
        self._logger.debug('{tag}Batch sampler initializing. process: {pid}; thread: {tid}'.format(tag=self._log_name_tag, pid=os.getpid(), tid=threading.current_thread().ident))
    ...
    def _setlabels(self, label_dist, label_mode, strict_selection, free_label_range):
        ...
        if self._data_adapter.mapping:
            # The label distribution keys must match the label mapper keys.
            #
            if not set(label_dist) == set(self._data_adapter.mapping):
                raise dptlabelerrors.LabelDistributionAndMappingMismatchError(label_dist, self._data_adapter.mapping)
        ...
        self._free_label_range = free_label_range

    def _setdataadapter(self, data_adapter):
        """
    ...
    def _setpatchshape(self, patch_shapes, mask_spacing, spacing_tolerance, input_channels):
        """
    ...
    def _addsource(self, patch_sources, create_stats, category_dist):
        """
        Add data source.
        ...
        for category_id, item_set in patch_sources.items():
            ...
            #
            self._data_source[category_id] = {'items': item_list, 'search': {label: set() for label in self._label_dist}}
            ...
            for item_index in range(len(item_list)):
                for label in self._label_dist:
                    if label in item_list[item_index].labels:
                        self._data_source[category_id]['search'][label].add(item_index)
        ...
        self._category_dist = {category_id: category_prob / category_dist_sum for category_id, category_prob in category_dist.items()}

    def _configuremultiprocessing(self, process_count, sampler_count, join_timeout, response_timeout, poll_timeout):
        """
        ...
        #
        self._sampler_count = sampler_count
        ...
        #
        if 0 < process_count:
            self._logger.debug('{tag}Multiprocessing enabled'.format(tag=self._log_name_tag))

            # Create 'spawn' type multiprocessing context. It is not the default on Linux.
            #
            self._mp_context = mp.get_context(method='spawn')
            ...
            self._logger.debug('{tag}Multiprocessing disabled'.format(tag=self._log_name_tag))
    ...
    def _startprocess(self):
        ...
        # Create new process.
        #
        pipe_cli, pipe_srv = self._mp_context.Pipe(duplex=False)
        process_name = 'patch sampler process {index}'.format(index=len(self._worker_processes))
        process_kwargs = {'command_pipe': pipe_cli, 'response_queue': self._response_queue, 'poll_timeout': self._poll_timeout}
        sampler_process = self._mp_context.Process(target=dptpatchsamplerdaemon.patchsampler_daemon_loop, name=process_name, kwargs=process_kwargs)
        ...
        #
        self._command_pipes[sampler_process.pid] = pipe_srv
        self._worker_processes[sampler_process.pid] = sampler_process

        self._logger.debug('{tag}Started sampler process: {pid}'.format(tag=self._log_name_tag, pid=sampler_process.pid))

        # Return the PID of the new process.
        #
        return sampler_process.pid

    def _checkprocess(self, process_id):
        """
        ...
        #
        if self._worker_processes[process_id].is_alive():

            # Ping the process to reset its wait timer.
            #
            self._command_pipes[process_id].send({'command': 'ping'})
            response_message = self._response_queue.get(block=True, timeout=self._response_timeout)

            if response_message['response'] != 'pong':
                self._logger.error('{tag}Checking sampler process {pid}: unexpected response: {response}'.format(tag=self._log_name_tag,
            ...
            self._logger.debug('{tag}Checking sampler process {pid}: alive and responding'.format(tag=self._log_name_tag, pid=process_id))

            return True

        # The process it not alive.
        #
        self._logger.error('{tag}Checking sampler process {pid}: process dead'.format(tag=self._log_name_tag, pid=process_id))

        return False

    def _waituntildone(self, count, command):
        """
        ...
        self._logger.debug('{tag}Waiting for confirmation for \'{command}\' command from {count} sampler process{plural}'.format(tag=self._log_name_tag,
                                                                                                                                 command=command,
                                                                                                                                 count=count,
        ...
        try:
            for _ in range(count):
                ...
                if response_message['response'] != 'done' or response_message['command'] != command:
                    self._logger.error('{tag}Unexpected response from {pid} sampler process for \'{command}\' command: {message}'.format(tag=self._log_name_tag,
                                                                                                                                         pid=response_message.get('pid', None),
                                                                                                                                         command=command,
                    ...
                    self._logger.debug('{tag}Sampler process {pid} is done with \'{command}\''.format(tag=self._log_name_tag,
                                                                                                      pid=response_message.get('pid', None),
                                                                                                      command=command))
            ...
            # No response in time.
            #
            self._logger.error('{tag}Not all responses arrived in time for \'{command}\' command'.format(tag=self._log_name_tag, command=command))

            raise dptprocesserrors.ProcessResponseTimeoutError(self._response_timeout, empty_queue_error)

    def _terminateprocess(self, process_id):
        """
        ...
        self._logger.debug('{tag}Stopping sampler process {pid}'.format(tag=self._log_name_tag, pid=process_id))

        # Sent stop command.
        #
        if self._worker_processes[process_id].is_alive():
            self._command_pipes[process_id].send({'command': 'stop'})

        # Close the connections.
        #
        self._command_pipes[process_id].close()

        # Wait for the process to stop. Terminate it forcefully if it does not exits in time.
        #
        self._worker_processes[process_id].join(timeout=self._join_timeout)
        if self._worker_processes[process_id].is_alive():
            self._worker_processes[process_id].terminate()

        # Remove process and pipe items from the dictionaries.
        #
        del self._command_pipes[process_id]
        del self._worker_processes[process_id]

        # Clear the patch sampler list.
        #
        deleted_source_items = [source_item for source_item in self._patch_samplers if self._patch_samplers[source_item] == process_id]
        for source_item in deleted_source_items:
            del self._patch_samplers[source_item]
    ...
    def _samplesources(self):
        ...
        category_minimum_counts = {category_id: 1 if len(self._data_source) <= category_population and 0.0 < self._category_dist[category_id] else 0 for category_id in self._category_dist}
        category_distribution = {category_id: (self._category_dist[category_id], category_minimum_counts[category_id], category_population) for category_id in self._category_dist}
        ...
        #
        if any(len(self._data_source[category_id]['items']) < category_source_count[category_id] for category_id in self._category_dist):
            available_image_counts = {category_id: len(self._data_source[category_id]['items']) for category_id in self._category_dist}
            self._logger.warning('{tag}The calculated {target} image distribution is not possible with {available} available images'.format(tag=self._log_name_tag,
        ...
        for category_id in self._category_dist:
            ...
            if category_source_count[category_id]:
                ...
                label_minimum_counts = {label: 1 if len(self._label_dist) <= label_population and 0.0 < self._label_dist[label] else 0 for label in self._label_dist}
                label_distribution = {label: (self._label_dist[label], label_minimum_counts[label], label_population) for label in self._label_dist}
        ...
        if self._strict_selection:
            ...
            if any(label not in available_labels_in_selection and 0.0 < self._label_dist[label] for label in self._label_dist):
                labels = list(self._label_dist.keys())
    ...
    def _patchdistribution(self, count):
        ...
        if self._strict_selection:
            ...
            #
            label_extract_ratios = {label: (self._label_dist[label], 0, count) for label in self._label_dist if 0.0 < self._label_dist[label]}
        else:
            # Strict selection checking is not enabled. It is possible that a label cannot be sampled from the current source set.
            #
            available_labels = set()
            for source_item in self._patch_samplers:
                available_labels.update(source_item.labels)

            label_extract_weights = {label: self._label_dist[label] for label in self._label_dist if label in available_labels and 0.0 < self._label_dist[label]}
            ...
            if label_extract_weights:
                ...
                #
                labels = list(self._label_dist.keys())
                sources = [source_item.stat if source_item.stat else source_item.mask for source_item in self._patch_samplers]
        ...
        for label in label_extract_counts:
            ...
            if 0 < label_count:
                ...
                #
                label_ratios = {source_item: (1.0 / source_count, 0, label_count) if label in source_item.labels else (0.0, 0, 0) for source_item in self._patch_samplers}
    ...
    def _stop(self):
        ...
        #
        if 0 < self._process_count:
            process_id_list = list(self._worker_processes.keys())
            for process_id in process_id_list:
                self._terminateprocess(process_id=process_id)
    ...
    def stop(self):
        ...
        #
        if 0 < self._process_count:
            self._logger.debug('{tag}Stopping all sampler processes'.format(tag=self._log_name_tag))
    ...
    def ping(self):
        ...
        if 0 < self._process_count:
            self._logger.debug('{tag}Checking sampler processes'.format(tag=self._log_name_tag))

            # Check if all the worker processes are alive and responding.
            #
            for process_id in self._worker_processes:
                if not self._checkprocess(process_id=process_id):
                    # The worker process is not alive.
                    #
                    self._logger.error('{tag}Process {pid} terminated before stepping'.format(tag=self._log_name_tag, pid=process_id))
    ...
    def step(self):
        ...
        self._logger.debug('{tag}Stepping patch sources'.format(tag=self._log_name_tag))
        ...
        if self._process_count == 0:
            ...
            for source_item in del_source_item_keys:
                self._logger.debug('{tag}Removing patch sampler for \'{source}\''.format(tag=self._log_name_tag, source=source_item.image))

                patch_sampler = self._patch_samplers.pop(source_item)
                patch_sampler.close()
                del patch_sampler
            ...
            for source_item in sorted(new_source_items):
                self._logger.debug('{tag}Adding patch sampler for \'{source}\''.format(tag=self._log_name_tag, source=source_item.image))

                self._patch_samplers[source_item] = dptpatchsampler.XmlPatchSampler(patch_source=source_item,
            ...
            #
            spawned_process_ids = []
            target_process_count = min(self._process_count, self._sampler_count, len(new_source_items))
            while len(self._worker_processes) < target_process_count:
                new_process_id = self._startprocess()
            ...
            for process_id in spawned_process_ids:
                ...
                self._logger.debug('{tag}Initializing sampler process: {pid}; with seed: {seed}; sending augmenters and label mappers'.format(tag=self._log_name_tag,
                                                                                                                                              pid=process_id,
                                                                                                                                              seed=process_random_seed))

                # Set the random seed for the processes.
                #
                self._command_pipes[process_id].send({'command': 'random', 'seed': process_random_seed})

                # Create the sampler in the process and store the host process identifier.
                #
                self._command_pipes[process_id].send({'command': 'hook', 'adapter': self._data_adapter})

            # Wait for the fresh patch sampler objects to initialize and the old ones to die.
            #
            if 0 < len(spawned_process_ids):
                self._waituntildone(count=len(spawned_process_ids), command='hook')
            ...
            #
            for source_item in del_source_item_keys:
                self._logger.debug('{tag}Removing source item \'{source}\' from sampler process {pid}'.format(tag=self._log_name_tag,
                                                                                                              source=source_item.image,
                                                                                                              pid=self._patch_samplers.get(source_item, None)))

                self._command_pipes[self._patch_samplers[source_item]].send({'command': 'remove', 'source': source_item})
                del self._patch_samplers[source_item]

            # Collect the number of images per worker processes.
            #
            image_counts = {process_id: 0 for process_id in self._worker_processes}
            for process_id in self._patch_samplers.values():
                image_counts[process_id] += 1

            # Wait fot the removed PatchSamplers to die.
            #
            self._waituntildone(count=len(del_source_item_keys), command='remove')
            ...
            for source_item in sorted(new_source_items):
                ...
                self._logger.debug('{tag}Adding source item \'{source}\' to sampler process {pid}'.format(tag=self._log_name_tag,
                                                                                                          source=source_item.image,
                                                                                                          pid=target_process_id))

                # Create the sampler in the process and store the host process identifier.
                #
                self._command_pipes[target_process_id].send({'command': 'create',
                                                              'source': source_item,
                                                              'spacing': self._mask_spacing,
                                                              'stat': self._create_stats,
                                                              'tolerance': self._spacing_tolerance,
                                                              'channels': self._input_channels,
                                                             'mode': self._label_mode})

                self._patch_samplers[source_item] = target_process_id

            # Wait for the fresh patch sampler objects to initialize.
            #
            self._waituntildone(count=len(new_source_items), command='create')
    ...
    def batch(self, batch_size):
        ...
        labels_dtype = np.object

        batch_dict = {spacing: {} for spacing in self._patch_shapes}
        for spacing in self._patch_shapes:
            patches_shape = (batch_size,) + self._patch_shapes[spacing] + (len(self._input_channels),)
            batch_dict[spacing]['patches'] = np.zeros(shape=patches_shape, dtype=patches_dtype)

            labels_shape = (batch_size,)

            batch_dict[spacing]['labels'] = np.empty(shape=labels_shape, dtype=labels_dtype)

            # Add weight arrays if necessary.
            #
            if self._data_adapter.weights:
        ...
        if self._process_count == 0:
            ...
            for source_item in sample_counts:
                ...
                self._logger.debug('{tag}Extracting patches of shapes {shape} with {dist} distribution from \'{source}\''.format(tag=self._log_name_tag,
            ...
            #
            waiting_process_set = set(self._command_pipes.keys()) - set(self._patch_samplers[source_item] for source_item in self._patch_samplers if source_item in sample_counts)
            for target_process_id in waiting_process_set:
                if not self._checkprocess(process_id=target_process_id):
                    # The worker process is not alive.
                    #
                    self._logger.error('{tag}Process {pid} terminated before patch extraction'.format(tag=self._log_name_tag, pid=target_process_id))
            ...
            #
            for source_item in sample_counts:
                target_process_id = self._patch_samplers[source_item]
                if self._worker_processes[target_process_id].is_alive():
                    self._logger.debug('{tag}Issuing extraction of patches of shapes {shape} with {dst} distribution to {pid} process from \'{src}\''.format(tag=self._log_name_tag,
                                                                                                                                                             shape=self._patch_shapes,
                                                                                                                                                             dst=sample_counts.get(source_item, None),
                                                                                                                                                             pid=target_process_id,
                                                                                                                                                             src=source_item.image))

                    self._command_pipes[target_process_id].send({'command': 'sample',
                                                                  'source': source_item,
                                                                  'counts': sample_counts[source_item],
                                                                  'shapes': self._patch_shapes})
                else:
                    self._logger.error('{tag}Sampler process {pid} terminated before sampling request'.format(tag=self._log_name_tag, pid=target_process_id))
            ...
            for _ in range(len(sample_counts)):
                ...
                #
                for process_id in self._worker_processes:
                    if not self._worker_processes[process_id].is_alive():
                        self._logger.error('{tag}Sampler process {pid} terminated before sampling response'.format(tag=self._log_name_tag, pid=process_id))
                ...
                try:
                    ...
                    # No response in time.
                    #
                    self._logger.error('{tag}Not all responses arrived in time for \'data\' command'.format(tag=self._log_name_tag))

                    raise dptprocesserrors.ProcessResponseTimeoutError(self._response_timeout, empty_queue_error)

                if response_message['response'] != 'data':
                    self._logger.error('{tag}Unexpected response from {pid} sampler process for \'data\' command: {message}'.format(tag=self._log_name_tag,
                                                                                                                                    pid=response_message.get('pid', None),
                                                                                                                                    message=response_message))
