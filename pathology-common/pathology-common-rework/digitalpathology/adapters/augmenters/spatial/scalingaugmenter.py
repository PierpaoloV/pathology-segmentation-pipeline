...
class ScalingAugmenter(dptspatialaugmenterbase.SpatialAugmenterBase):
    ...
    def transform(self, patch):
        ...
        if self.__scaling_factor < 1.0:
            ...
            pad_widths = (patch.shape[0] * pad_ratio, patch.shape[1] * pad_ratio)
            pad_config = ((math.ceil(pad_widths[0]), math.ceil(pad_widths[0])), (math.ceil(pad_widths[1]), math.ceil(pad_widths[1])), (0, 0))

            patch_padded = np.pad(array=patch, pad_width=pad_config[:patch.ndim], mode='reflect')
        else:
            patch_padded = patch

        # Zoom patch.
        #
        zoom_config = (self.__scaling_factor, self.__scaling_factor, 1.0)
        order = 0 if patch.ndim < 3 else self.__interpolation_order
        patch_transformed = scipy.ndimage.zoom(input=patch_padded, zoom=zoom_config[:patch.ndim], order=order, mode='reflect')
        ...
        if patch_transformed.shape != patch.shape:
            border = (math.floor((patch_transformed.shape[0] - patch.shape[0]) / 2.0), math.floor((patch_transformed.shape[1] - patch.shape[1]) / 2.0))
            patch_transformed = patch_transformed[border[0]:border[0]+patch.shape[0], border[1]:border[1]+patch.shape[1]]
