...
class ImageWriter(object):
    ...
    def __configurecache(self, image_path, cache_path):
        ...
        if cache_path is None:
            ...
            # Avoid caching on existing file.
            #
            self.__cache = None if os.path.isfile(cache_target) else cache_target
    ...
    def __setparameters(self, dtype, coding, indexed_channels, compression, interpolation, tile_size, jpeg_quality):
        ...
        self.__tile_shape = (tile_size, tile_size, channel_count)
    ...
    def __rightbottompad(self, tile):
        ...
        return np.pad(array=tile,
                      pad_width=((0, self.__tile_shape[0] - tile.shape[0]), (0, self.__tile_shape[1] - tile.shape[1]), (0, 0)),
                      mode='constant',
    ...
    def fill(self, content):
        ...
        Raises:
            ...
            ContentChannelsMismatchError: The content dimensions does not match the configured image channels.
        ...
        if content.ndim < 3 and 1 < self.__channels or 3 <= content.ndim and content.shape[2] != self.__channels:
            raise dptimageerrors.ContentChannelsMismatchError(content.shape, self.__channels)

        # Write out the image content tile by tile.
        #
        for row in range(0, content.shape[0], self.__tile_size):
            for col in range(0, content.shape[1], self.__tile_size):
    ...
    def write(self, tile, row, col):
        ...
        are padded. Tiles are expected in channels last order.
        ...
        elif tile.ndim == len(self.__tile_shape) and tile.shape[-1] == self.__channels or tile.ndim == len(self.__tile_shape) - 1 and self.__channels == 1:
            # Add dimension if necessary.
            #
            write_tile = tile if tile.ndim == len(self.__tile_shape) else tile[..., None]

            # Check if the tile is filled with the empty value.
            #
            if not self.__skip_empty or not np.array_equal(write_tile, self.__empty_tile):
                # Write out the given tile with padding if necessary.
                #
                if write_tile.shape != self.__tile_shape:
                    self.__writer.writeBaseImagePartToLocation(self.__rightbottompad(tile=write_tile).flatten(), col, row)
                else:
                    self.__writer.writeBaseImagePartToLocation(write_tile.flatten(), col, row)
        else:
