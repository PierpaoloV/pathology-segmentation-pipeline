...
class SimpleSampler(object):
    ...
    def __init__(self,
        ...
        Args:
            ...
            copy_config (dict): Automatically copy all data using a mapping defined by a dictionary. Example: "{'root':'/home/user/data'}", will copy data to a local drive, and use it.
        """
    ...
    def _setsource(self, source_path, partition):
        ...
        if self._copy_config is not None:
            items = dptimagefile.copy_batch_source(batch_source=batch_source, target_replacements=self._copy_config,
                                                   source_replacements=None, purposes=[partition], categories=None, 
                                                   allow_missing_stat=self._create_stats, overwrite=False)
            self._logger.info('Copied: {} items, Skipped: {} items'.format(items[0], items[1]))
            batch_source.update(path_replacements=self._copy_config)
    ...
    def _map_classes(self, mask):
        max_key = max(self._label_map.keys())
        lut = np.zeros(max_key + 1, dtype=mask.dtype)
    ...
    def step(self):
        ...
        if len(self._patch_samplers) >= source_count:
    ...
    def __getitem__(self, item):
        ...
        weights = np.asarray(np.isin(mask, list(self._label_dist.keys())), dtype=np.float32)
        if self._label_map:
