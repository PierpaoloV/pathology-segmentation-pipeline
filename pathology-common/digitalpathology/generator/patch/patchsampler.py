...
class PatchSampler(dptbasepatchsampler.BasePatchSampler):
    ...
    def __init__(self, patch_source, create_stat, mask_spacing, spacing_tolerance, input_channels, label_mode, cache_path=None):
        """
        ...
        super().__init__()
        self._image = None             # Image patch loader.
    ...
    def _openimage(self, image_path, mask_path, input_channels):
        ...
        self._image = dptimagereader.ImageReader(image_path=image_path,
                                                 spacing_tolerance=self._spacing_tolerance,
                                                 input_channels=input_channels,
                                                 cache_path=self._cache_path)

        if mask_path:
            self._mask = dptimagereader.ImageReader(image_path=mask_path,
                                                    spacing_tolerance=self._spacing_tolerance,
                                                    input_channels=[0],
                                                    cache_path=None)
            ...
            if None in self._mask.spacings:
                for image_level in range(len(self._image.shapes)):
                    if self._image.shapes[image_level] == self._mask.shapes[0]:
                        self._mask.correct(spacing=self._image.spacings[image_level], level=0)
                        break

            # Check if the mask spacings are correct now: the spacings for all levels are valid.
            #
            if None in self._mask.spacings:
                raise dptimageerrors.UnfixableImageSpacingError(self._image.path, self._image.shapes, self._mask.path, self._mask.shapes)

        self._input_channel_count = self._image.channels
    ...
    def _collectdata(self, stat_path, create_stat, mask_spacing, mask_labels):
        ...
        matching_level = None
        for image_level in range(len(self._image.shapes)):
            if self._image.shapes[image_level] == self._stats.shape and abs(self._image.spacings[image_level] - self._stats.spacing) < self._spacing_tolerance:
        ...
        if matching_level is not None:
            ...
            self._stats_downsamplings = [self._image.downsamplings[matching_level] / level_downsampling for level_downsampling in self._image.downsamplings]
            self._stats_shifts = [max(0, int(stats_downsampling) // 2 - 1) for stats_downsampling in self._stats_downsamplings]
        else:
            raise dptimageerrors.ImageShapeMismatchError(self._image.path, self._image.shapes, self._image.spacings, self._stats.path, self._stats.shape, self._stats.spacing)
        ...
        if self._mask:
            ...
            if matching_level is not None and \
               self._image.shapes[matching_level] == self._mask.shapes[mask_level] and \
               abs(self._image.spacings[matching_level] - self._mask.spacings[mask_level]) < self._spacing_tolerance:
                ...
                self._mask_level = [min(max(0, level - level_diff), len(self._mask.shapes) - 1) for level in range(len(self._image.shapes))]
                self._mask_downsamplings = [self._image.downsamplings[level] / self._image.downsamplings[self._mask_level[level] + level_diff] for level in range(len(self._image.shapes))]
                self._mask_shifts = [max(0, int(mask_downsampling) // 2 - 1) for mask_downsampling in self._mask_downsamplings]
            else:
                raise dptimageerrors.ImageShapeMismatchError(self._image.path,
                                                             self._image.shapes,
                                                             self._image.spacings,
                                                             self._mask.path,
    ...
    def single_sample(self, item, label, shape, spacing):
        ...
        level = self._image.level(spacing=spacing)
    ...
    def _get_patch(self, central_coordinate, level, shape, spacing):
        ...
        patch = self._image.read(spacing=spacing,
